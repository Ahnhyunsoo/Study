...
HRESULT CGraphic_Device::InitDevice(HWND hWnd, _uint iWinSizeX, _uint iWinSizeY, GRAPHICDESC::WINMODE eWinMode, LPDIRECT3DDEVICE9* ppOut)
{
	...
	*ppOut = m_pDevice; // LPDIRECT3DDEVICE 가 포인터인데 이거의 포인터니까 이중포인터이고 이중포인터의 값이니 주소값이된다.
...
{
	Safe_Release(m_pFont); //릴리즈 순서는 중요하다. Font는 Sprite가필요하고 Sprite는 Device가 필요하기때문이다.
