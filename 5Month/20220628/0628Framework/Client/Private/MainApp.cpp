...
void CMainApp::Free()
{
	Safe_Release(m_pGameInstance); //싱글톤의 래퍼런스카운트를 공유받은후에 Release를 해주니 객체가삭제가안되고 카운트가감소한다.

	CGameInstance::Release_Engine(); // 이때 이 함수를 호출해주면 래퍼런스카운트가0인 싱글톤들의 Release가 호출되어 객체가 삭제된다.
}
