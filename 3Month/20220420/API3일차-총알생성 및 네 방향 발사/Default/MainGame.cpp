...
#pragma endregion 복습
	...
	//m_MList.push_back(CMonster());
	if (!m_pMonster)
	{
		m_pMonster = new CMonster();
		m_ObjList[OBJ_MONSTER].push_back(m_pMonster);
	}
	dynamic_cast<CPlayer*>(m_ObjList[OBJ_PLAYER].front())->Set_BulletList(&m_ObjList[OBJ_BULLET]);
	dynamic_cast<CMonster*>(m_ObjList[OBJ_MONSTER].front())->Initialize();
}
...
#pragma endregion 복습
	for (int i = 0; i < OBJ_END; ++i)
	{
			...
			iter != m_ObjList[i].end(); )
		{
			int iResult = (*iter)->Update();
			RECT rc;
			bool a = IntersectRect(&(*iter)->GetRect(), &(*iter)->GetRect(), &(*iter)->GetRect());
	...
	/*for (auto iter = m_MList.begin(); iter != m_MList.end();)
	{	
		iter->Update();
		if (iter->GetM().left < WINCX-700)
		{
			iter->SetDir(DIR_RIGHT);
		}
		else if (iter->GetM().right > WINCX - 100)
		{
			iter->SetDir(DIR_LEFT);		
		}
		++iter;
