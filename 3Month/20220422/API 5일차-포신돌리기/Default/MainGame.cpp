...
// 		iter->Late_Update();*/
	...
	for (auto& iter = m_ObjList[OBJ_MONSTER].begin(); iter != m_ObjList[OBJ_MONSTER].end();)
	{
			...
			/*Player 의 세타를 알아야한다 세타만 넘겨주면된다
			세타를 구하는공식 = acos 아트코사인에 인자로 빗변과 밑변을 넣어주면된다.
			높이와 밑변을 알면 빗변을 구할 수 있다. 피타고라스의 정리 
			내가 구해야하는건 플레이어의 빗변이므로 
			플레이어의 높이와 밑변을 구해야한다. 이미있지 밑변 = fCX 높이 = fCY 
			C = fCX*fCX + fCY*fCY
			루트 벗기는함수 뭐였지 */
		float C = sqrtf((m_ObjList[OBJ_PLAYER].front()->Get_Info().fCX
			*m_ObjList[OBJ_PLAYER].front()->Get_Info().fCX)
			+ (m_ObjList[OBJ_PLAYER].front()->Get_Info().fCY
				* m_ObjList[OBJ_PLAYER].front()->Get_Info().fCY)
		);
		dynamic_cast<CMonster*>(*iter)->Set_xy(acos(m_ObjList[OBJ_PLAYER].front()->Get_Info().fCX / C));
		++iter;
	}
	/*for (auto& iter : m_ObjList[OBJ_MONSTER])
	{
		iter->Set_Pos(m_ObjList[OBJ_PLAYER].front()->Get_fX(), m_ObjList[OBJ_PLAYER].front()->Get_fY());
	}*/
